...
logger = logging.getLogger("fetch-free-proxy")
...
def check(proxy):
    ...
    url = "https://www.ele.me/shop/804119"
...
def fetch_all(endpage=2, https=False,checked=True):
    proxyes = []
    ...
    if checked:
        ...
        for p in proxyes:
            if check(p):
                logger.debug("proxy [{}] is valid".format(p))
...
if __name__ == '__main__':
    ...
    proxyes = fetch_all(checked=True)
    #print check("202.29.238.242:3128")
    for p in proxyes:
        print(p)

    # proxy_handler = urllib.request.ProxyHandler({'http': "http://223.241.118.198:8010"})
    # opener = urllib.request.build_opener(proxy_handler, urllib.request.HTTPHandler)
    #
    # response = opener.open("https://www.ele.me/shop/804119",timeout=5)
    # print(response.code)
